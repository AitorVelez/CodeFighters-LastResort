...
Grenade::Grenade(int x, int y, int HP) : Enemy(x, y, HP)
{
	...
	original_x = x;
	original_y = y;
	collider = App->collision->AddCollider({ 0, 0, 14, 14 }, COLLIDER_TYPE::COLLIDER_PLAYER_SHOT, (Module*)App->enemies);
...
{	
	...
	if (original_y > App->player->position.y && position.y == original_y) {
		flag = true;
		Bullets = &shot;
